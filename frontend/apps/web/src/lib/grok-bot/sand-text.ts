/**
 * Vendored from Grok Bot 0.16.0: shared text-shaping primitives.
 */
export function clampLine(raw: string, maxLength: number): string {
  return raw.replace(/\s+/g, " ").trim().slice(0, maxLength);
}

export function clampBlock(raw: string, maxLength: number): string {
  return raw.trim().slice(0, maxLength);
}

export function decapitalize(phrase: string): string {
