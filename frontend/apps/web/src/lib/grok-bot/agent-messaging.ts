// Vendored from the user's Grok Bot 0.16.0 agent-to-agent messaging core.
import { clampBlock, clampLine } from "./sand-text";
import { GROUP_CHAT_TAG_PREFIX } from "./group-chat";
...
export interface SandAgentAddress {
  readonly id: string;
...
export interface SandAgentGroupInfo {
  readonly id: string;
  readonly name: string;
  readonly members: readonly SandAgentAddress[];
}
...
export const AGENT_MESSAGE_MAX_TEXT_LENGTH = 8000;
export const AGENT_NAME_MAX_LENGTH = 72;
export const AGENT_DESCRIPTION_MAX_LENGTH = 2000;

const AGENT_DIRECTORY_PROMPT_LIMIT = 40;

export function clampAgentMessage(text: string): string {
  return clampBlock(text, AGENT_MESSAGE_MAX_TEXT_LENGTH);
}

export function clampAgentName(name: string): string {
  return clampLine(name, AGENT_NAME_MAX_LENGTH);
}

export function clampAgentDescription(description: string): string {
  return clampBlock(description, AGENT_DESCRIPTION_MAX_LENGTH);
}

function describeAddress(address: SandAgentAddress): string {
  const description =
    address.description != null && address.description.trim().length > 0
...
export function renderAgentDirectorySystemPrompt(
  others: readonly SandAgentAddress[],
  groups: readonly SandAgentGroupInfo[] = [],
...
export function buildAgentInboundWakePrompt(args: {
  readonly from: SandAgentAddress;
  ...
  if (images.length > 0) {
    ...
    for (const image of images) {
      const alt =
        image.alt != null && image.alt.trim().length > 0 ? ` — ${clampLine(image.alt, 200)}` : "";
...
export function buildMentionedAgentsContext(mentioned: readonly SandAgentAddress[]): string | null {
